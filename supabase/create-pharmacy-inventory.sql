...
-- Allow public read access (everyone can see the inventory for now)
CREATE POLICY "Anyone can view pharmacy inventory"
    ON public.pharmacy_inventory FOR SELECT
    USING (true);

-- Only admins can modify (this will be set via service role key)
CREATE POLICY "Admins can modify pharmacy inventory"
    ON public.pharmacy_inventory FOR ALL
    USING (false)
    WITH CHECK (false);

-- Add 50 demo medicines
INSERT INTO public.pharmacy_inventory (name, category, description, price, discount_percentage, stock_quantity, supplier_info)
VALUES
