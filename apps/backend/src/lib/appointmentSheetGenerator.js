 ...
 * Creates a professional appointment sheet for when patients book sessions
 */
...
export function generateAppointmentSheetPDF(appointmentData) {
  const {
    ...
    hospitalInfo = {
      ...
      address: "[Full Address], Peshawar, Pakistan",
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      doc.fontSize(14).font('Helvetica').text('Appointment Sheet', 50, 75, { align: 'center' });
...
export function generateAppointmentSheetFileName(patient, appointmentId) {
  const date = new Date().toISOString().split('T')[0];
  const patientName = (patient?.name || 'Unknown').replace(/\s+/g, '_').toLowerCase();
  return `appointment-sheet_${patientName}_${appointmentId}_${date}.pdf`;
}
